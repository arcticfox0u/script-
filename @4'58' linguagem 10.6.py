...
# -----------------------------------------
# [ ENCRYPTION CORE v10.6 - THE FINAL BOSS ]
# [ by healingboy85 ]
# [ STATUS: BRANDED HEADER COMPLETE ]
# -----------------------------------------
...
while True:
    ...
    # CABEÇALHO COM SEU NOME: BY: HEALINGBOY85
    print(f"{cor_menu}  ╔" + "═"*45 + "╗")
    ...
    # MODO TRADUZIR TEXTO (PERSISTENTE)
    if msg == "♰":
        while True:
            os.system("clear")
            ...
            print(f"\033[1;30m  (Cole o texto e aperte ENTER + CTRL+D para gerar)\n")
            
            entrada_longa = sys.stdin.read().splitlines()
            
            if len(entrada_longa) == 1 and entrada_longa[0] == "♰":
                break
                
            if entrada_longa:
                res_lista = processar(entrada_longa, ti if modo_inverso else t)
                tit_resultado = "PROJETO ENCRIPTADO" if modo_inverso else "TEXTO TRADUZIDO"
                mostrar_resultado(res_lista, cor_menu, tit_resultado)
            else:
                break
        continue

    # MODO TURBO (RESPOSTA RÁPIDA)
    if msg.strip() != "":
